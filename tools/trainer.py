...
def main():
    ...
    cfg_da_source = cfg_source.clone()
    cfg_da_source.DATASETS.TRAIN = ("2019_da",)
    cfg_da_source.DATASETS.VAL = ("2019_da_val",)
    cfg_da_source.DATASETS.TEST = ("2019_da_val",)

    cfg_da_source_val = cfg_da_source.clone()
    cfg_da_source_val.DATASETS.TRAIN = cfg_da_source.DATASETS.VAL

    cfg_da_target = cfg_da_source.clone()
    cfg_da_target.DATASETS.TRAIN = ("2021_da",)
    cfg_da_target.DATASETS.VAL = ("2021_da_val",)
    cfg_da_target.DATASETS.TEST = ("2021_da_val",)

    cfg_da_target_val = cfg_da_target.clone()
    cfg_da_target_val.DATASETS.TRAIN = cfg_da_target.DATASETS.VAL
    ...
    with EventStorage(start_iter) as storage:
        for data_source, data_target, data_source_val, data_da_source, data_da_target, data_da_source_val, data_da_target_val, iteration in zip(data_loader_source, data_loader_target, data_loader_source_val, data_loader_da_source, data_loader_da_target, data_loader_da_source_val, data_loader_da_target_val, range(start_iter, max_iter)):
            ...
            if da_bool:
                ...
                if comm.is_main_process():
                    storage.put_scalars(feature_extractor_loss=losses_reduced, **loss_dict_reduced)
            ...
            with torch.no_grad():
                ...
                losses_reduced = sum(loss for loss in loss_dict_reduced.values())
                if comm.is_main_process():
                    storage.put_scalars(feature_extractor_val_loss=losses_reduced, **loss_dict_reduced)
            ...
            if iteration - start_iter > 5 and ((da_bool and iteration % 20 == 0) or iteration == max_iter):
